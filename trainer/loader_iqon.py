    ...
    def __getitem__(self, idx):
        ...
        if conf["repeated_interact"]:
            ...
            while int(pos_bottom_idx.numpy()) in bottom_his:
                bottom_his.remove(int(pos_bottom_idx.numpy()))
        if conf["num_his"] <= len(bottom_his):
            ...
            value, idx = torch.topk(b_score, k= conf["num_his"], dim=-1) #tensor([597.7301, 588.4199, 544.7933]) tensor([84, 52, 49])
            ...
            ub_his = []
            # if conf["popular_padding"]:
            for i in range(conf["num_his"]):
            ...
            for i in range(conf["num_his"]-len(ub_his)):
        ...
        if conf["num_his"] <= len(top_his):
            ...
            value, idx = torch.topk(t_score, k= conf["num_his"], dim=-1) #tensor([597.7301, 588.4199, 544.7933]) tensor([84, 52, 49])
            #此处的idx指的是当前user——bottom his （单条数据）对应的u_idx的第84个bottom 的idx，需要还原到item_map里面对应的bottomidx
            ...
            for i in range(conf["num_his"]-len(ut_his)):
        ...
        if int(top_idx.numpy()) in self.top_bottoms_dict: #top 交互历史可能为0
            ...
            if conf["repeated_interact"]:
                ...
                while int(pos_bottom_idx.numpy()) in top_bottoms:
                    top_bottoms.remove(int(pos_bottom_idx.numpy()))
            if conf["num_interact"] <= len(top_bottoms): 
                bottom_v_fea = self.visual_features[pos_bottom_idx]  
                ...
                value_2, idx_2 = torch.topk(b2_score, k= conf["num_interact"], dim=-1) 
                tb_his = [top_bottoms[int(i.numpy())] for i in idx_2]
           
            elif len(top_bottoms) == 0:
                tb_his = []  
                # if conf["popular_padding"]: 
                for i in range(conf["num_interact"]):
                ...
                for i in range(conf["num_interact"] -len(tb_his)):
